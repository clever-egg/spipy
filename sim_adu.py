...
class simulation():
	...
	def generate_pattern(self, euler_angles, patt, verbose):
		...
		dr_projectionn = {}                    # project the molecule onto the xOy plane, len = pat_num
		...
		for ind,euler_angle in enumerate(euler_angles):
			...
			pic_size = int(2 * abs(np.round(coor[:,0:2])).max())
			dr_projection = np.zeros((pic_size + 1,pic_size + 1))
			projection = (np.round(coor[:,0:2]) + (pic_size/2)).astype(int)
			...
			dr_projectionn[ind] = dr_projection                                     		

			del dr_projection
			del pat
			gc.collect()
			print('Done. Time (s): '+str(time.time()-time0))
		return patt, dr_projectionn															
